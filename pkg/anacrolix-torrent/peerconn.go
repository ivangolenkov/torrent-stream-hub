...
func (c *PeerConn) onReadExtendedMsg(id pp.ExtensionNumber, payload []byte) (err error) {
	...
	case utHolepunch.ExtensionName:
		...
		if err != nil {
			// Ignoring BEP 55 unmarshal errors to prevent dropping connections
			// (some clients send Bencode instead of the expected binary blob).
			err = nil
			return
		}
...
func makeBuiltinLtepProtocols(pex bool) LocalLtepProtocolMap {
	// Disabled utHolepunch.ExtensionName to prevent peering issues
	ps := []pp.ExtensionName{pp.ExtensionNameMetadata}
	if pex {
